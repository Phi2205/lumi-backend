...
export class PresenceService {
  ...
  async markOnline(userId: string, socketId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Error marking user ${userId} online: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async markOffline(userId: string, socketId: string): Promise<boolean> {
    try {
      ...
      const remainingSocketsCount = await this.redisService
        .getClient()
        .scard(socketsKey);
      ...
      this.logger.error(
        `Error marking user ${userId} offline: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async handleHeartbeat(userId: string): Promise<void> {
    try {
      ...
      const isStillOnline = await this.redisService.expire(
        presenceKey,
        this.HEARTBEAT_TTL,
      );
      ...
      this.logger.error(
        `Error handling heartbeat for user ${userId}: ${error.message}`,
      );
    }
  }
  ...
  async isOnline(userId: string): Promise<boolean> {
    try {
      ...
      this.logger.error(
        `Error checking status for user ${userId}: ${error.message}`,
      );
      return false;
    }
  }
  ...
  async getLastOnline(userId: string): Promise<string | null> {
    try {
      ...
      this.logger.error(
        `Error getting last online for user ${userId}: ${error.message}`,
      );
      return null;
    }
  }
